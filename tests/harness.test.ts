...
// Mock worktree 模块（保持 mock.module：仅本文件使用，无跨文件干扰）
...
beforeEach(() => {
	...
	mockCreateWorktree.mockImplementation(
		async (slug: string) => `/tmp/test-wt-${slug}`,
	);
...
test("worktree isolation: 启用时创建 worktree 并透传 cwd，无变更自动清理", async () => {
	// 子Agent 直接返回文本（无工具调用）
	transport.push({ content: '{"result": "ok"}', tool_calls: undefined });
...
test("worktree isolation: 有变更时保留 worktree 并报告路径和变更文件", async () => {
	mockGetChanges.mockImplementation(async (_path) => [
		"modified.txt",
		"new_file.ts",
	]);

	transport.push({ content: '{"result": "done"}', tool_calls: undefined });
...
test("worktree isolation: createWorktree 失败时返回 error", async () => {
	mockCreateWorktree.mockImplementation(async (_slug: string) => {
		throw new Error("git 失败");
	});
...
test("worktree isolation: 不启用 isolation 时不影响正常 dispatch", async () => {
	transport.push({ content: '{"result": "normal"}', tool_calls: undefined });
...
test("getPlanMessages: 两文件都不存在 → 返回空数组", async () => {
	...
	try {
		await Bun.spawnSync(["rm", "-rf", "plans"]);
	} catch {}
