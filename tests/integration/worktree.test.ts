...
import { createWorktree, getChanges, removeWorktree } from "../../src/worktree";
...
afterAll(async () => {
	...
	const list = Bun.spawnSync(["bash", "-c", "git worktree list --porcelain"], {
		cwd: REPO_ROOT,
	});
	if (list.exitCode === 0) {
	...
	for (const entry of Bun.spawnSync(["bash", "-c", "ls .relay/worktrees/"], {
		cwd: REPO_ROOT,
	})
		.stdout.toString()
		.split("\n")
		...
		if (entry.startsWith("test-int-") && existsSync(full)) {
			try {
				await removeWorktree(full);
			} catch {
				try {
					Bun.spawnSync(["bash", "-c", `rm -rf "${full}"`]);
				} catch {}
			}
		}
	}
...
test("创建 worktree → 目录存在且是 git 仓库", async () => {
	...
	const stat = Bun.spawnSync([
		"bash",
		"-c",
		`test -f "${path.join(wtPath, ".git")}" && echo file || echo not-file`,
	]);
