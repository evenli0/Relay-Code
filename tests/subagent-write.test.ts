...
import type { DispatchConfig } from "../src/types";
import { ScriptedLLM } from "./helpers/mock-llm";

// ── Mock LLM（可注入 transport，Phase1 E-1）──────────
let transport: ScriptedLLM;

beforeEach(() => {
	transport = new ScriptedLLM([
		{ content: "mock fallback", tool_calls: undefined },
	]);
	setMockTransport(transport);
});

afterAll(() => setMockTransport(null));
...
function makeConfig(opts?: Partial<DispatchConfig["prompt"]>): DispatchConfig {
	return {
		prompt: {
			...
			task:
				opts?.task ??
				"分析 src/tools.ts 的代码结构，把报告写入 docs/analysis.md",
...
function logMessages(
	label: string,
	msgs: { role: string; content?: string | null; tool_calls?: unknown[] }[],
): void {
	console.log(`\n=== ${label} ===`);
	for (const m of msgs) {
		console.log(
			`[${m.role}] ${typeof m.content === "string" ? m.content.slice(0, 200) : "(tool_calls)"}`,
		);
	}
	console.log("=== end ===\n");
}
...
test("write 工具：缺路径 → 返回可操作的错误提示", async () => {
	...
	// 缺 path
	const r1 = await executor.executeToolCall("write", {
	...
	// 空 path
	const r2 = await executor.executeToolCall("write", {
		path: "",
		content: "some content",
	});
	...
	const r3 = await executor.executeToolCall("write", {
		path: "test_empty.md",
		content: "",
	});
...
test("完整流程：子 Agent 读文件 → write 报告 → JSON 汇报", async () => {
		...
		tool_calls: [
			{
				...
				function: {
					name: "read",
					arguments: JSON.stringify({ path: "src/tools.ts" }),
				},
			},
		],
	});
	...
	transport.push({
		content: JSON.stringify({
			keyFindings: ["tools.ts 定义了读/写/grep/bash/dispatch"],
			summary: "已完成分析并写入 docs/test_analysis.md",
		}),
	...
	try {
		...
		if (existsSync("docs/test_analysis.md"))
			unlinkSync("docs/test_analysis.md");
	} catch {
		/* ignore */
	}
});
...
test("write 缺路径后 LLM 在下一轮能修正", async () => {
		...
		tool_calls: [
			{
				...
				function: {
					name: "write",
					arguments: JSON.stringify({ path: "", content: "# 报告\n内容" }),
				},
			},
		],
	});
	...
	// Round 3: JSON 汇报
	transport.push({
		content: JSON.stringify({ keyFindings: ["完成"], summary: "done" }),
	});
	...
	try {
		const { unlinkSync, existsSync } = await import("node:fs");
		if (existsSync("docs/report.md")) unlinkSync("docs/report.md");
	} catch {
		/* ignore */
	}
});
